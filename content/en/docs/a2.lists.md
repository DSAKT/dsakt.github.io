...
Custom objects can also be initialized using the `List()` constructor:

```kotlin
val studentList = List(3) { index -> Student("Student ${index + 1}", 80 + index * 5) }
println(studentList)
// Output: [Student(name=Student 1, grade=80), Student(name=Student 2, grade=85), Student(name=Student 3, grade=90)]
```
