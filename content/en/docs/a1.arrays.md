...
You can create an array to hold strings using `arrayOf()`.

```kotlin
val stringArray = arrayOf("Kotlin", "Java", "Swift")
println(stringArray[1])  // Output: Java
```

---

#### **6. Creating an Array of Custom Objects**
To work with custom objects, define a class and then use `arrayOf()` or the `Array` constructor.

```kotlin
data class Student(val name: String, val id: Int, val age: Int)

val studentArray = arrayOf(
    Student("Alice", 101, 20),
    Student("Bob", 102, 22),
    Student("Charlie", 103, 19)
)

println(studentArray[0])  // Output: Student(name=Alice, id=101, age=20)

// Accessing properties of a Student object
println(studentArray[1].name)  // Output: Bob
```

Here, we define a `Student` data class with properties `name`, `id`, and `age`, and create an array of `Student` objects.
...
```kotlin
val arr = arrayOf(5, 3, 2, 4, 1)
val sortedArr = arr.sorted()
println(sortedArr.joinToString())  // Output: 1, 2, 3, 4, 5
```
...
println(sortedArr.joinToString())  // Output: 5, 4, 3, 2, 1
```
...
println(reversedArr.joinToString())  // Output: 5, 4, 3, 2, 1
```

---

#### **13. `joinToString()`**
Joins the elements of the array into a single string with a specified delimiter.
...
#### **14. `map()`**
Applies a transformation function to each element in the array and returns a new list.

```kotlin
val arr = arrayOf(1, 2, 3)
val squared = arr.map { it * it }
println(squared.joinToString())  // Output: 1, 4, 9
```
...
```kotlin
val arr = arrayOf(1, 2, 3, 4, 5)
println(arr.average())  // Output: 3.0
```

---

#### **16. `all()`**
Checks if all elements in the array satisfy a given predicate.

```kotlin
val arr = arrayOf(2, 4, 6)
println(arr.all { it % 2 == 0 })  // Output: true
```

---

#### **17. `any()`**
Checks if any element in the array satisfies a given predicate.

```kotlin
val arr = arrayOf(1, 3, 5)
println(arr.any { it % 2 == 0 })  // Output: false
```
